...
(function(){
	...
	phantomFlow.
		...
		listen('uniqueStep.end', function(e){
			...
			e.node.testUrl = this.currentUrl;
			var diffFile = visualResultsRoot + currentFilename + "/" + currentFlowName + "/" + pathName + ".diff.png";
			if (!fs.isFile(diffFile)) {
			    e.node.screenshot = null;
			    console.log("CaptureFailed: " + diffFile);
			}
			casper.then(function(){debug(pathName);});
			casper.emit('test.step.complete', {name: pathName });
		}).		
        listen('step.skip', function (e) {
            var pathName = e.decisions.map(function (i) { return safe(i); }).join('/') + '/' + safe(e.name);
            e.node.screenshot = currentFilename + '/' + pathName + ".png";
            e.node.testUrl = this.currentUrl;
            console.log('step.skip: ' + pathName);
		});
