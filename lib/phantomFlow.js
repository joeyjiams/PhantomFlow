...
function runStaticTests(staticTests) {
    ...
    for (var i = 0; i < staticTests.length; i++) {
        var branch = function (url, name, selector, browserWidths, hideSelector, owner, signInFunc) {
            return function () { registerStaticTestsInVariousWidth(url, name, selector, browserWidths, hideSelector, owner, signInFunc) };
        };
        tests[staticTests[i].name] = branch(staticTests[i].url, staticTests[i].name, staticTests[i].selector, staticTests[i].browserWidths, staticTests[i].hideSelector, staticTests[i].owner, staticTests[i].SignInFunction);
    }

    chance(tests);
}

function registerStaticTestsInVariousWidth(url, name, selector, browserWidths, hideSelector, owner, signInFunc) {
    ...
    for (var j = 0; j < browserWidths.length; j++) {
        ...
        var teststep = function (url, name, selector, browserWidth, hideSelector, owner, signInFunc) {
            return function () { step(name, staticTestFactory(url, name, selector, browserWidth, hideSelector, signInFunc), owner); };
        };
        testBranches[widthBranch] = teststep(url, testname, selector, browserWidths[j], hideSelector, owner, signInFunc);
    }

    chance(testBranches);
}

function staticTestFactory(url, name, selector, browserWidth, hideSelector, signInFunc) {
    return function () {
        if(typeof(signInFunc) === 'function') {
            signInFunc();
        }
		var urlToOpen = typeof(url) === "function"? url() : url;
        casper.thenOpen(urlToOpen, function () {
